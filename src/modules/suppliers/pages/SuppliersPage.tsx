...
const SortIcon = () => (
  <svg width="12" height="12" viewBox="0 0 20 16" fill="none">
    <path
      d="M9 13L5 17L1 13M5 17V1M9 1H13M9 5H16M9 9H19"
      stroke="#08283B"
      strokeWidth="2"
      strokeLinecap="round"
...
const SkeletonRow: React.FC = () => (
  <tr className="bg-canvas-50 animate-pulse">
    <td className="px-4 h-20 align-middle border-b border-canvas-300">
      <div className="h-4 bg-canvas-300 rounded w-3/4" />
    </td>
    <td className="px-4 h-20 align-middle border-b border-canvas-300">
      <div className="h-6 bg-canvas-300 rounded-full w-16" />
    </td>
    <td className="px-4 h-20 align-middle border-b border-canvas-300">
      <div className="h-4 bg-canvas-300 rounded w-1/2 mb-2" />
      <div className="h-3 bg-canvas-300 rounded w-3/4" />
    </td>
    <td className="px-4 h-20 align-middle border-b border-canvas-300">
      <div className="h-6 bg-canvas-300 rounded w-36" />
    </td>
    <td className="px-4 h-20 align-middle border-b border-canvas-300">
      <div className="h-8 bg-canvas-300 rounded w-8 mx-auto" />
    </td>
  </tr>
);
...
const SuppliersPage: React.FC = () => {
  ...
  const error = suppliersError ? (suppliersError as Error).message : '';
  ...
  const filtered = suppliers.filter(s => {
    ...
    if (q) {
      const haystack = [s.companyName ?? '', s.fullName, s.email].join(' ').toLowerCase();
  ...
  const thClass =
    'px-4 py-3 text-left text-[13px] font-semibold text-navy-600 whitespace-nowrap bg-canvas-300 border-b border-canvas-300 select-none';
                ...
                <div className="absolute top-full mt-1 left-0 z-50 bg-surface-card border border-stroke-medium rounded-lg shadow-[0_4px_16px_rgba(0,0,0,0.10)] py-1 min-w-[160px]">
                  {statusOptions.map(opt => (
                    <button
                      ...
                      className={`block w-full text-left px-4 py-2.5 text-sm cursor-pointer hover:bg-canvas-100 transition-colors ${statusFilter === opt.value ? 'font-semibold text-brand-navy' : 'text-text-primary'}`}
                    >
                      {opt.label}
                    </button>
                  ))}
                      ...
                      <button
                        ...
                        onClick={e => {
                          ...
                          if (openMenuId === s.id) {
                            ...
                            openMenuFor(s.id, e.currentTarget);
                          }
                        }}
        ...
        {!loading && (
          <div className="flex items-center justify-between px-5 py-3 border-t border-canvas-300 gap-3 flex-wrap">
            <span className="text-[13px] text-navy-600 whitespace-nowrap">
              {t('suppliers.list.showing', { start: rangeStart, end: rangeEnd, total: filtered.length })}
            </span>
              ...
              <div className="flex items-center gap-1">
                ...
                {buildPageNumbers(safePage, totalPages).map((p, idx) =>
                  typeof p === 'string' ? (
                    <span
                      key={p + idx}
                      className="inline-flex items-center justify-center min-w-8 h-8 px-1 text-[13px] text-navy-300"
                    >
                      …
                    </span>
                  ) : (
